...
#define LEFT_HAND_BULLET_SPPED 0.07f
#define LEFT_HAND_ATTACK_COUNT 1
#define LEFT_HAND_ANIMATE_RATE	5
...
CLeft_Hand_Boss::CLeft_Hand_Boss() :CEnemy(OBJ_BOSS_STAGE2_HAND, 0, 0, 100, 100)
{
	ChangeState(LEFT_APPEAR);
	_Time_Wait = 0;
	Create();
}
CLeft_Hand_Boss::CLeft_Hand_Boss(int Type, float X, float Y, float Width, float Height) :CEnemy(Type, X, Y, Width, Height)
{
	ChangeState(LEFT_APPEAR);
	_Time_Wait = 0;
	Create();
}
CLeft_Hand_Boss::~CLeft_Hand_Boss()
{

}

void CLeft_Hand_Boss::Update(float delta)
{
	if (_Live_State == DESTROYED) return;
	if (_Live_State == ALIVE) ProcessAlive(delta);
}

void CLeft_Hand_Boss::ProcessAlive(float delta)
{
...
void CLeft_Hand_Boss::UpdateCollision()
{
...
void CLeft_Hand_Boss::ProcessStone()
{
	switch (_State_Left)
	{
	case LEFT_SWING:
	...
	case LEFT_HUNG:
		Hung();
		break;
	case LEFT_FINISH_MOVE:
		Finish();
		break;
	case LEFT_LOCK_TARGET:
		LockTarget();
		break;
	case LEFT_BEFORE_SWING:
		Prepare();
		break;
	default:
		break;
...
void CLeft_Hand_Boss::ProcessState()
{
	...
	case LEFT_SWING:
		...
		{
			ChangeState(LEFT_SPIN);
		}
		break;
	case LEFT_APPEAR:
		if (now - _Last_State > TIME_APPEAR)
		{
			ChangeState(LEFT_BEFORE_SWING);
		}
		break;
	case LEFT_SPIN:
		if (now - _Last_State > TIME_SPIN)
		{
			ChangeState(LEFT_HUNG);
		}
		break;
	case LEFT_HUNG:
		if (now - _Last_State > TIME_HUNG)
		{
			ChangeState(LEFT_FINISH_MOVE);
		}
		break;
	case LEFT_FINISH_MOVE:
		if (now - _Last_State > TIME_FINISH_MOVE)
		{
			ChangeState(LEFT_LOCK_TARGET);
		}
		break;
	case LEFT_LOCK_TARGET:
	...
	case LEFT_BEFORE_SWING:
		...
		{
			ChangeState(LEFT_SWING);
		}
		break;
	}
}

void CLeft_Hand_Boss::Prepare()
{
	float angle = 1.0f;
...
void CLeft_Hand_Boss::Appear()
{
	...
	for (int i = NUM_STONE - 1; i > 0; i--)
	{
		...
		if (_List_Stone.at(i)->_Y_Center > _List_Stone.at(0)->_Y_Center + STONE_WIDTH *sin(_List_Stone.at(i)->_Angle))
		{
...
void CLeft_Hand_Boss::LockTarget()
{
	ProcessAttack(LEFT_HAND_ATTACK_COUNT, TIME_BETWEEN_ATTACK, LEFT_HAND_ANIMATE_RATE, false);
...
void CLeft_Hand_Boss::ChangeState(STATE_LEFT_HAND state)
{
	_State_Left = state;
	_Last_State = GetTickCount();
	if (_State_Left != LEFT_APPEAR)
	{
		_R = STONE_WIDTH;
		_List_Stone.at(0)->_IsUp = true;
	}
	if (_State_Left == LEFT_SWING)
	{
	...
	if (_State_Left == LEFT_FINISH_MOVE)
	{
		_List_Stone.at(0)->_IsUp = false;
	}
	if (_State_Left == LEFT_LOCK_TARGET)
	{
		_List_Stone.at(0)->_IsUp = true;
	}
	if (_State_Left == LEFT_SPIN)
	{
		_List_Stone.at(0)->_IsUp = true;
	}
}

void CLeft_Hand_Boss::Create()
{
...
D3DXVECTOR3 CLeft_Hand_Boss::CheckTarget()
{
	D3DXVECTOR3 result;
	float angle = _List_Stone.at(NUM_STONE - 1)->Find_Angle();
	result.x = LEFT_HAND_BULLET_SPPED * cos(angle);
	result.y = LEFT_HAND_BULLET_SPPED * sin(angle);
	result.z = 0;
	return result;
}

D3DXVECTOR2 CLeft_Hand_Boss::Find_XY()
{
	D3DXVECTOR2 result;
